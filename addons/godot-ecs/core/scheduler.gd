...
var _system_pool: Dictionary[GDScript, ECSParallel]
var _system_graph: Dictionary[GDScript, Array]
var _system_conflict: Dictionary[GDScript, Dictionary]
var _system_group: Dictionary[GDScript, int]
...
func add_systems(systems: Array) -> ECSScheduler:
	for sys: ECSParallel in systems:
		...
		_system_pool[sys.get_script() as GDScript] = sys
	for sys: ECSParallel in systems:
...
## @param key: System class that must run first.
## @param value: System class that depends on key (must run after key).
func _insert_graph_node(key: GDScript, value: GDScript) -> void:
...
## @param name: System class.
## @param before_systems: Array of system classes that must run after this system.
func _set_system_before(name: GDScript, before_systems: Array) -> void:
	for key: GDScript in before_systems:
		_insert_graph_node(name, key)

## Internal: Callback for systems declaring their after dependencies.
## @param name: System class.
## @param after_systems: Array of system classes that must run before this system.
func _set_system_after(name: GDScript, after_systems: Array) -> void:
	for key: GDScript in after_systems:
		_insert_graph_node(key, name)

## Internal: Stores component access conflict information for a system.
## @param name: System class.
## @param table: Dictionary mapping component classes to access modes.
func _set_system_conflict(name: GDScript, table: Dictionary[GDScript, int]) -> void:
	_system_conflict[name] = table

## Internal: Stores scheduling group assignment for a system.
## @param name: System class.
## @param group: Integer group identifier.
func _set_system_group(name: GDScript, group: int) -> void:
...
func _build_batch_systems() -> void:
	DependencyBuilder.new()\
		...
		_batch_systems.append(batch_system_keys.map(func(key: GDScript):
...
class DependencyBuilder extends RefCounted:
	var _graph: Dictionary[GDScript, Array] = {}
	var _conflict: Dictionary[GDScript, Dictionary] = {}
	var _group: Dictionary[GDScript, int] = {}

	## Sets the dependency graph (DAG).
	## @param graph: Dictionary mapping system classes to arrays of dependent system classes.
	## @return: This builder for method chaining.
	func with_dag(graph: Dictionary[GDScript, Array]) -> DependencyBuilder:
		_graph.merge(graph, true)
		return self

	## Sets the component access conflict table.
	## @param conf: Dictionary mapping system classes to component access patterns.
	## @return: This builder for method chaining.
	func with_conflict(conf: Dictionary[GDScript, Dictionary]) -> DependencyBuilder:
		_conflict.merge(conf, true)
		return self

	## Sets the system group assignments.
	## @param group: Dictionary mapping system classes to group IDs.
	## @return: This builder for method chaining.
	func with_group(group: Dictionary[GDScript, int]) -> DependencyBuilder:
		_group.merge(group, true)
		return self

	## Builds execution batches and invokes callback for each batch.
	## @param catch: Callable that receives Array of system classes for each batch.
	func build(catch := Callable()) -> void:
		var all_systems: Array[GDScript] = _group.keys()
		var in_degree: Dictionary[GDScript, int] = {}
		
		for key: GDScript in all_systems:
			in_degree[key] = 0
			
		for u: GDScript in _graph:
			for v: GDScript in _graph[u]:
				if in_degree.has(v):
					in_degree[v] += 1
					
		var ready_queue: Array[GDScript] = []
		for key: GDScript in all_systems:
			if in_degree[key] == 0:
		...
		while processed_count < total_count:
			...
			var current_batch: Array[GDScript] = []
			var batch_reads: Dictionary[GDScript, bool] = {}
			var batch_writes: Dictionary[GDScript, bool] = {}
			var next_loop_deferred: Array[GDScript] = []
			var unlocked_nodes: Array[GDScript] = []
			
			for sys_name: GDScript in ready_queue:
			...
			for executed_sys: GDScript in current_batch:
				if _graph.has(executed_sys):
					for neighbor: GDScript in _graph[executed_sys]:
	...
	## @param sys_name: System class to check.
	...
	func _is_conflict(sys_name: GDScript, batch_reads: Dictionary[GDScript, bool], batch_writes: Dictionary[GDScript, bool]) -> bool:
	...
	## @param sys_name: System class.
	## @param batch_reads: Dictionary to record read accesses.
	## @param batch_writes: Dictionary to record write accesses.
	func _mark_access(sys_name: GDScript, batch_reads: Dictionary[GDScript, bool], batch_writes: Dictionary[GDScript, bool]) -> void:
		var sys_access: Dictionary = _conflict.get(sys_name, {})
		for comp: GDScript in sys_access:
			var access_type: int = sys_access[comp]
	...
	## @param arr: Array of system classes to sort.
	func _sort_by_group(arr: Array[GDScript]) -> void:
