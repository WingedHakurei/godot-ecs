...
# [核心 1] 声明权限
# 告诉调度器：我要读写 CompHealth 组件，只读 CompRegen 组件
func _list_components() -> Dictionary[GDScript, int]:
    return {
        CompHealth: ECSParallel.READ_WRITE, # 可能修改血量
        CompRegen: ECSParallel.READ_ONLY # 只读取恢复速度
    }
...
*注意：`ECSRunner.set_system_update` 仅对 `ECSSystem` 有效，对调度器管线内的 `ECSParallel` 无效。*
