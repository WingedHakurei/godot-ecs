...
*(注意：所有组件都必须自行在 `_on_pack` / `_on_unpack` 中实现字段的序列化，框架不会自动处理。) *
...
**⚠️ 特殊说明：含节点引用的组件序列化**
如果组件内存储的是 Godot 场景节点引用（例如某个组件持有一个 `Node`），请注意：**节点引用无法序列化**。
通常的做法是：

1. 不保存这类组件（或在 `_on_pack` 中跳过节点字段）。
...
框架内置了 `tests/test_suite.gd`。如果你修改了框架源码，或者怀疑框架行为不符合预期，可以运行它。
