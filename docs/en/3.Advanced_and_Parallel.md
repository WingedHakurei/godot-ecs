...
# Keys are component classes (GDScript)
func _list_components() -> Dictionary[GDScript, int]:
    return {
        CompHealth: ECSParallel.READ_WRITE, # May modify health
        CompRegen: ECSParallel.READ_ONLY # Only read regeneration rate
    }
...
# Arg 1: view (Dictionary containing components of current entity, keyed by component class)
...
func _view_components(view: Dictionary, cmds: ECSParallel.Commands) -> void:
    var hp = view[CompHealth] as CompHealth
    var rate = view[CompRegen] as CompRegen
    ...
    if hp.data < hp.max_value:
        ...
        if hp.data >= hp.max_value:
            ...
            cmds.entity(view.entity.id()).remove_component(CompRegen)
```
...
func _ready() -> void:
    ...
    # [Key] Define Dependencies (classes, not strings)
    # Movement must be after Input
    sys_move.after([SysInput])
    # Physics must be after Movement
    sys_physics.after([SysMovement]) 
    # Render must be after Physics
    sys_render.after([SysPhysics])
