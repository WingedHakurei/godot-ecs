...
var healths = world().view(CompHealth)
for hp in healths:
    print((hp as CompHealth).data)
```
...
* **Caching Mechanism**: The framework uses `QueryCache` internally. It builds a cache upon the first request for `[CompPos, CompVel]`. Subsequent frame calls have nearly zero overhead. Adding/removing components automatically updates the cache.
...
var results = world().multi_view([CompPos, CompVel])

for res in results:
    var entity = res["entity"]           # Get entity reference
    var pos = res[CompPos] as CompPos    # Get component instance
    var vel = res[CompVel] as CompVel
...
var dying_entities = world().query() \
    .with([CompHealth]) \
    .without([CompDebuff]) \
    .filter(func(data): return (data[CompHealth] as CompHealth).data < 50) \
    .exec()
...
```gdscript
# [Incorrect Example]
var list = world().view(CompHealth)
for hp in list:
    if (hp as CompHealth).data <= 0:
...
cmds.spawn() \
    .add_component(CompHealth.new(100)) \
    .add_component(CompPos.new(0, 0))

# 3. Add/Remove components for existing entity
cmds.entity(entity_id) \
    .add_component(CompBuff.new()) \
    .remove_component(CompDebuff)

# 4. Send deferred event (Also for thread safety)
cmds.notify("enemy_spawned", 1)

# 5. Deferred operation (executes on the main thread).
cmds.defer(func():
    print("run on main thread.")
)
```
