...
*(Note: All components are plain `ECSComponent` subclasses; serialization is always implemented in `_on_pack`/`_on_unpack`.)*
...
**⚠️ Special Note: Serialization of Node-Bound Components**
Components that hold references to Godot scene nodes (declared with `var view: Node`, for example) **cannot serialize node references**.
The usual practice is:

1. Do not save the node-bound component (skip it in `_on_pack`, or re-create it after loading).
...
The framework comes with `tests/test_suite.gd`. If you modify the framework source code or suspect unexpected behavior, you can run it.
